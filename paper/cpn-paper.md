...
The approach described here uses marine refrigeration hardware — a Vitrifrigo ND50 compressor unit with a Danfoss variable-speed compressor and a stainless-steel evaporator plate mounted inside the terrarium. Marine refrigeration is a distinct hardware category from the general "compressor cooling" approaches above (chest-freezer conversions, aquarium chillers, modified portable air conditioners): the unit ships as a complete factory-sealed split system — compressor, condenser, refrigerant lines, evaporator plate — sized for boat refrigerators and small marine cold-storage cabinets. Three practical consequences are worth highlighting for hobbyist replicators. First, **installation is mechanical, not refrigeration work**: the condenser end mounts externally, the evaporator plate bolts inside the enclosure, and the pre-formed refrigerant lines run between the two — no field plumbing of the cold loop, no system charging. Second, **the unit class is customizable to enclosure size**: marine-refrigeration vendors offer compressor/evaporator combinations spanning roughly 30–200 L cold-storage capacity with several evaporator footprints, so terraria between ~0.3 m³ and ~3 m³ can be matched without custom engineering. Third, **the refrigerant loop ships pre-charged and factory-sealed**: installation does not require an F-gas certification in EU/UK jurisdictions because no work is performed on the refrigerant circuit (the certification requirement applies to filling or topping up the loop, not to installing a sealed unit). This removes a significant skills barrier that has historically restricted DIY compressor-based terrarium cooling to chest-freezer conversions or to growers willing to pay for a refrigeration engineer's time. This class of equipment, designed for boat refrigeration where compact size, low power draw (31 W), 12/24 V DC operation, and reliable performance in confined humid spaces are essential, has not previously been applied to terrarium cooling. The evaporator plate doubles as a condensation surface, and its large area (1220 × 280 mm) provides gentle, distributed cooling rather than the point-source cold spots of coiled tubing or radiators. The system routinely drives the terrarium to 13.5 deg C — approximately 3 deg C below the evaporative cooling floor and nearly 9 deg C below room ambient — while operating silently and drawing less power than a standard light bulb.
